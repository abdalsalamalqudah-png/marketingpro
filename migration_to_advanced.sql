-- Migration script to upgrade to advanced schema
-- Adding missing columns and tables for advanced features

-- Add lead_score column to clients table if it doesn't exist
ALTER TABLE clients ADD COLUMN lead_score INTEGER DEFAULT 0;

-- Add priority column to clients table if it doesn't exist
ALTER TABLE clients ADD COLUMN priority TEXT DEFAULT 'medium' CHECK (priority IN ('low', 'medium', 'high'));

-- Add stage column to clients table for sales pipeline
ALTER TABLE clients ADD COLUMN stage TEXT DEFAULT 'lead' CHECK (stage IN ('lead', 'contacted', 'qualified', 'proposal', 'negotiation', 'closed_won', 'closed_lost'));
...
CREATE TABLE IF NOT EXISTS email_sequence_steps (
    ...
    conditions TEXT, -- JSON for conditions
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS whatsapp_business (
    ...
    features TEXT DEFAULT '{}', -- JSON for enabled features
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (team_id) REFERENCES teams(id) ON DELETE CASCADE
);

-- Add conversation_id to whatsapp_messages for better organization
ALTER TABLE whatsapp_messages ADD COLUMN conversation_id TEXT;
ALTER TABLE whatsapp_messages ADD COLUMN message_type TEXT DEFAULT 'text' CHECK (message_type IN ('text', 'image', 'document', 'audio', 'video', 'template'));
...
CREATE TABLE IF NOT EXISTS ai_insights (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    team_id INTEGER NOT NULL,
    insight_type TEXT NOT NULL, -- 'campaign_recommendation', 'content_suggestion', etc.
    title TEXT NOT NULL,
    description TEXT,
